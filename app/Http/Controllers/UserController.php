...
class UserController extends Controller
{
    ...
    {
        $param = $request->all();
        $users = User::with('files')->where('role', User::ROLE_EDITOR) //relationship từ file để chạy thay cho các vòng for vì nếu sư dụng vòng for cho từng ng sẽ sập server
            ->orderBy('id', 'DESC')
    ...
    public function store(Request $request)
    {
        $param = $request->all();
        // Validate
        $checkData = UserValidate::validate($request);
        $user = new User();
